...
def read_original_data(fname):
    ...
    with open(fname, 'r') as fr:
        ...
        for i, line in enumerate(lines):
            ...
            data.append(' | '.join([line['prev'], line['future']]))
...
def score_by_bert(A, hyps, B, model, tokenizer, device='cuda'):
    ...
    """
    def _score(a, b):
        encoded = tokenizer.encode_plus(a, text_pair=b, return_tensors='pt', max_length=512)
        for k in encoded:
...
def _truncate_text(text):
    ...
    if '.' not in text and '?' not in text and '!' not in text:
        ...
        if len(seperate) >5:
            ...
            word = seperate[round((len(seperate)-1)/2)]
        #print(word)
        strt = text.find(word)
        #print(text[space])
        
        tol_len = len(text)
        for i in range(strt, tol_len):
...
def _truncate_text(text):
    ...
    if '.' not in text and '?' not in text and '!' not in text:
        ...
        if period_index<0:
            period_index = _find_nth_overlapping(text, '?', 2)
        if period_index < 0 and first_period_index!=-1:
            return text[:first_period_index+1]
        if period_index < 0 and first_exclamation_index!=-1:
            return text[:first_period_index+1]
        if period_index < 0 and first_q_index!=-1:
            return text[:first_period_index+1]
...
def main():
    ...
    parser.add_argument("--output_file", type=str, default='./outputs', help="Output dir")
    ...
    write_text(ranked_hypotheses, output_file)
